...
void CTextureManager::Update()
{
	...
	m_freeTextureCreators = m_updateTextureCreators;
	m_updateTextureCreators.clear();
}

void CTextureManager::CopyFirstMip()
{
	...
	for (unsigned int i = 0; i < m_updateTextureCreators.size(); ++i)
	{
		...
		imgBarries[i] = m_updateTextureCreators[i]->GetImage()->CreateMemoryBarrier(VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
			VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_ASPECT_COLOR_BIT);
	}

	vk::CmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, (uint32_t)imgBarries.size(), imgBarries.data());
}

void CTextureManager::GenerateMips()
{
	//VkCommandBuffer cmdBuffer = vk::g_vulkanContext.m_mainCommandBuffer;

	std::vector<VkImageMemoryBarrier> mipsBarriers;
	for (unsigned int i = 0; i < m_updateTextureCreators.size(); ++i)
		m_updateTextureCreators[i]->GenerateMips();
}
...
void TextureCreator::GenerateMips()
{
	VkCommandBuffer cmdBuff = vk::g_vulkanContext.m_mainCommandBuffer;
	ImageHandle* img = m_texture->m_image;

	VkImageSubresourceLayers sourceResource; //first mip level
	cleanStructure(sourceResource);
	sourceResource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
	sourceResource.baseArrayLayer = 0;
	sourceResource.layerCount = 1;
	sourceResource.mipLevel = 0;

	//change mips layout to transfer_dst
	std::vector<VkImageMemoryBarrier> layoutBarriers;

	for (uint32_t i = 1; i < DEFAULT_MIPLEVELS; ++i)
		layoutBarriers.push_back(img->CreateMemoryBarrierForMips(i, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 0, VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_ASPECT_COLOR_BIT));

	vk::CmdPipelineBarrier(cmdBuff, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, (uint32_t)layoutBarriers.size(), layoutBarriers.data());
	...
	{
		VkImageBlit imgBlit;
		cleanStructure(imgBlit);
		imgBlit.srcSubresource = sourceResource;
		...
		VkExtent3D dim = img->GetDimensions();
		int32_t height = (dim.height > 1) ? dim.height >> i: 1; //its 1d image.have to do the same for z > 1
		imgBlit.srcOffsets[1].x = dim.width;
		imgBlit.srcOffsets[1].y = dim.height;
		imgBlit.srcOffsets[1].z = 1;
		imgBlit.dstOffsets[1].x = dim.width >> i;
		imgBlit.dstOffsets[1].y = height;
		imgBlit.dstOffsets[1].z = 1;

		blits.push_back(imgBlit);
	}

	vk::CmdBlitImage(cmdBuff,
		img->Get(),
	...
	//change layout for shader usage

	VkImageMemoryBarrier finalTransition = img->CreateMemoryBarrierForMips(0, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, 0, VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_ASPECT_COLOR_BIT, DEFAULT_MIPLEVELS);

	vk::CmdPipelineBarrier(cmdBuff, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, nullptr, 0, nullptr, 1, &finalTransition);
}
...
void CTexture::CreateTexture(const SImageData& imageData, bool ownData)
{
	...
	imgTextInfo.mipLevels = DEFAULT_MIPLEVELS;
	...
	imgTextInfo.usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
