   ...
   )
{
	...
	for (; (P.x * stepDirection < end) && (stepCount <= maxSteps); P += dP, k += dK, Q += dQ, stepCount += 1.0f)
	{
		...
		if (rayZMax >= (sceneZMax - 150.0f) && rayZMin <= sceneZMax)
		{
			hit = true;
			break;
		}
	...
	//debug = texelFetch(Color_T, ivec2(permute ? P1.yx : P1), 0);
	
	//vec4 clipPoint = ViewToScreenSpace * vec4(rayOrigin, 1.0f);
	
	debug = vec4(P, sceneZMax, stepCount);
	//return !(OutsideOfScreenSpace(hitCoords.x) || OutsideOfScreenSpace(hitCoords.y));
	return hit;
}
...
void main()
{
	...
	vec3 normalVS = normalize(vec3(ViewMatrix * vec4(normalWS, 1.0f)));
	...
	ssrOutput = vec4(1.0f, 0.0f, 1.0f, 1.0f);
	if (hit)
